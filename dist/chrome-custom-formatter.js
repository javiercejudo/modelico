...
var customFormatter = (function (M) {
  ...
  var _header = function _header(obj) {
    ...
    try {
      subtypeNames = '<' + parentType.innerTypes()[key.slice(0, -FIELDS_POSTFIX.length)].subtypes.map(function (x) {
  ...
  var withMethods = function withMethods(obj) {
    ...
    var res = Object.keys(base).reduce(function (acc, key) {
      var keyName = obj instanceof M.List ? 'get(' + key + ')' : key === ':' || Array.isArray(json) ? key : key + FIELDS_POSTFIX;
  ...
  var body = function body(obj) {
    ...
    if (fieldsKeys.length === 0) {
      children = [['div', { style: styles.childObject }, ['object', { object: fields }]]];
    } else {
