...
const reviverFactory$4 = (keyMetadata, valueMetadata) => (k, v, path = []) => {
  ...
  return new EnumMap(new Map(Object.keys(v).map(parseMapper$1(keyReviver, valueReviver, v, path))));
};
...
const jsonSchemaMetadata = validate => {
  ...
  const jscMapImpl = (keyMetadata, valueMetadata, schema) => {
    ...
    const keyValueSchemaGetter = () => ({
      items: Object.assign({ items: [getInnerSchema(keyMetadata), getInnerSchema(valueMetadata)] }, baseSchema.items)
    });
