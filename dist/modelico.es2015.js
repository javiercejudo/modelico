...
const getSchemaImpl = metadata => {
  ...
  const innerTypes = hasInnerTypes ? getInnerTypes$1([], metadata.type) : emptyObject;
...
var asIs = ((transformer = identity) => Object.freeze({
  type: transformer,
...
const reviverFactory$5 = itemMetadata => (k, v, path) => {
  ...
  const revive = v === null ? always(null) : defaultTo(metadata.reviver)(metadata.maybeReviver);
...
const raw_ = (Type, innerMetadata) => Type.metadata ? Type.metadata(...innerMetadata) : base(Type);
...
const formatError = (ajv, schema, value, path = []) => ['Invalid JSON at "' + path.join(' -> ') + '". According to the schema\n', JSON.stringify(schema, null, 2) + '\n', 'the value (data path "' + ajv.errors.filter(e => e.dataPath !== '').map(error => error.dataPath) + '")\n', JSON.stringify(value, null, 2) + '\n'].concat(ajv.errors.map(error => error.message)).join('\n');

const formatDefaultValueError = (ajv, schema, value) => ['Invalid default value. According to the schema\n', JSON.stringify(schema, null, 2) + '\n', 'the default value\n', JSON.stringify(value, null, 2) + '\n'].concat(ajv.errors.map(error => error.message)).join('\n');
...
var ajvMetadata = ((ajv = { validate: T }) => {
  ...
  ajvMetadata.ajv_ = (Type, schema = emptyObject, innerMetadata, topLevel = false) => {
    const metadata = _(Type, innerMetadata);

    return ajvMeta(metadata, emptyObject, schema, () => getSchema(metadata, topLevel));
  };

  ajvMetadata.ajvBase = (Type, schema = emptyObject, topLevel = false) => {
    const metadata = base(Type);

    return ajvMeta(metadata, { type: 'object' }, schema, () => getSchema(metadata, topLevel));
  };

  ajvMetadata.ajvAsIs = (schema, transformer = identity) => ajvMeta(asIs(transformer), schema);
  ...
  ajvMetadata.ajvBoolean = schema => ajvMeta(boolean(), { type: 'boolean' }, schema);

  ajvMetadata.ajvDate = schema => ajvMeta(date(), { type: 'string', format: 'date-time' }, schema);
  ...
  const ajvList = (schema, itemMetadata) => ajvMeta(list(itemMetadata), { type: 'array' }, schema, () => ({
  ...
  ajvMetadata.ajvList = (schema, itemMetadata) => Array.isArray(itemMetadata) ? ajvTuple(schema, itemMetadata) : ajvList(schema, itemMetadata);
  ...
  ajvMetadata.ajvStringMap = (schema, valueMetadata) => ajvMeta(stringMap(valueMetadata), { type: 'object' }, schema, () => ({
    additionalProperties: false,
    patternProperties: {
  ...
  ajvMetadata.ajvSet = (schema, itemMetadata) => ajvMeta(set(itemMetadata), {
    type: 'array',
    uniqueItems: true
  }, schema, () => ({ items: getSchema(itemMetadata, false) }));

  ajvMetadata.ajvMaybe = itemMetadata => ajvMeta(maybe(itemMetadata), emptyObject, emptyObject, () => getSchema(itemMetadata, false));
  ...
  ajvMetadata.ajvAnyOf = (conditionedMetas, enumField) => ajvMeta(anyOf(conditionedMetas, enumField), {
...
const createModel = (innerTypes = emptyObject, { base = Base$1, stringTag = 'ModelicoModel', metadata: m = metadata$2 } = {}) => {
