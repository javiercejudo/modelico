...
const parseMapperFactory = (keyReviver, valueReviver, path) => (pair, i) => [keyReviver('', pair[0], path.concat(i, 0)), valueReviver('', pair[1], path.concat(i, 1))];
...
const reviverFactory$2 = (kMetadata, vMetadata) => (k, v, path = []) => {
  if (k !== '') {
    return v;
  }
  ...
  const reviveFromObject = () => objToArr(v).map((pair, i) => [JSON.parse(pair[0]), pair[1]]).map(parseMapper);

  return ModelicoMap.fromMapWithOptions({ asArray }, new Map(asArray ? reviveFromArray() : reviveFromObject()));
};
...
const tupleItemMetadataGetter = (itemMetadata, v, path) => i => isFunction(itemMetadata[i]) ? itemMetadata[i](v, path) : itemMetadata[i];
...
const iterableReviverFactory = (IterableType, itemMetadata) => (k, v, path = []) => {
  ...
  const itemMetadataGetter = isTuple ? tupleItemMetadataGetter(itemMetadata, v, path) : isFunction(itemMetadata) ? _ => itemMetadata(v, path) : _ => itemMetadata;
...
const jsonSchemaMetadata = validate => {
  ...
  const _jscImpl = (Type, innerMetadata, schema, topLevel) => {
    ...
    return jscMeta(metadata, emptyObject, schema, () => getSchema(metadata, topLevel));
  };
  ...
  const jscBaseImpl = (Type, schema, topLevel) => {
    ...
    return jscMeta(metadata, { type: 'object' }, schema, () => getSchema(metadata, topLevel));
  };
...
const reviverFactory$6 = enumerators => (k, v, path = [], { default: def } = {}) => {
...
const maybeReviverFactory = enumerators => {
  const reviver = reviverFactory$6(enumerators);

  return (k, v, path) => enumerators[v] === undefined ? null : reviver(k, v, path);
};
